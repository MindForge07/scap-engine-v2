...
### 7.1 复验（5 runs，2026-08-14）：深度增益未复现，聚焦指令有害

`analogy-exp.ps1 -Runs 5`（45 任务 + 9 批盲评，`parse-judge.py` 汇总，n=15/组）：

| 组 | quality | depth | E2 迁移（限流任务） |
|---|---|---|---|
| A 基线 | 3.73 | 4.20 | 0/15 |
| B 聚焦指令 | 3.53 | 3.67 | 5/15 |
| C 无指令 | **4.20** | 4.20 | 5/15 |

1. **迁移确凿且复现**：E2（测输入→显式上限→先于上限行动→先丢低优先级）B/C 5/15 vs A 0/15——本质经验跨域迁移稳定存在
2. **深度增益是 2-runs 小样本噪声**：5-runs 下 B depth 3.67 < A 4.20（-0.53）；2-runs 的 +0.5 未复现
3. **聚焦指令有害（两次一致）**：B 两次实验均为最低（4.17/3.53），C 两次最高（4.50/4.20）——与 Gick & Holyoak 人类实验相反：LLM 看到案例即自动使用（无需提示），而「先分析如何解决再迁移」的指令诱导照搬类比、挤占领域工程细节
4. **若产品化，注入方式用 C（纯案例参考无指令）**，不用 B（聚焦指令）

**最终判定**：聚焦类比 = **策略多样性通道**（防教科书锚定、改变思考方向，证据确凿），**不是效率提升通道**（质量/深度无增益）。保持默认关闭；若未来启用，用 C 式注入且定位为「多样性增强」而非「效率提升」。采集机制见 `essence-case-design.md`。
